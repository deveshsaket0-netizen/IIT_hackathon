      ...
      {items.map(({ key, label, path }) => {
        ...
        return (
          <button
            ...
            className="group flex flex-col items-center gap-1 px-2 sm:px-4 py-1 rounded-lg transition-all duration-200 disabled:opacity-40 enabled:hover:-translate-y-0.5"
            style={{ color: isActive ? colors.fern : colors.inkMuted }}
          >
            <Icon
              className="w-5 h-5 transition-all duration-200 group-enabled:group-hover:drop-shadow-[0_0_8px_currentColor]"
              style={{ color: isActive ? colors.fern : "inherit" }}
            />
            <span className="text-[10px] tracking-wide transition-colors duration-200 group-enabled:group-hover:text-[#6fae6a]">
              {label}
            </span>
          </button>
        );
